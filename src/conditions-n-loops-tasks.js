...
function shuffleChar(str, iterations) {
  ...
  if (size <= 1 || iterations === 0) return str;
  ...
  for (cycle = 1; cycle <= iterations; cycle += 1) {
  ...
  const effectiveIterations = iterations % cycle;
...
function getNearestBigger(number) {
  ...
  if (pivot === digits.length - 1) return number;
  ...
  [digits[pivot], digits[minMax]] = [digits[minMax], digits[pivot]];

  for (i = 0; i < pivot; i += 1) {
    for (let j = i + 1; j < pivot; j += 1) {
